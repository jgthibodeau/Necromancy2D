        ...
        foreach (var collection in m_objectRenderers)
        {
            ...
            foreach (var render in collection.Key)
            {
				...
				if (render is MeshRenderer) {
					subMeshCount = render.gameObject.GetComponent<MeshFilter> ().mesh.subMeshCount;
				} else if (render is SkinnedMeshRenderer) {
					subMeshCount = ((SkinnedMeshRenderer)render).sharedMesh.subMeshCount;
				}

				for (int i = 0; i < subMeshCount; i++) {
					m_commandBuffer.DrawRenderer (render, m_highlightMaterial, i, (int)collection.Value.sortingType);
				}
            }
        }
