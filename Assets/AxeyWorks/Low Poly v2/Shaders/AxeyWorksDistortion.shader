...
Shader "AxeyWorks/Distortion" 
{
	Properties 
	{
		...
		_Lustre("Matte", Float) = 1.0
		_Churn("Wave Churn", Float) = 0.5
	...
	SubShader 
	{
    	...
    	Pass 
    	{
			...
			float _Height;
			float _Speed;
			float _RandHeight = 0.5;
			float _RandSpeed = 0.5;
			
			uniform float4 _LightColor0;

			uniform float4 _MainColour;
			uniform float4 _SpecularColour;
			uniform float _Lustre;
			...
			void geom(triangle v2g IN[3], inout TriangleStream<g2f> triStream)
			{
				...
				float3 ambientLighting =
					UNITY_LIGHTMODEL_AMBIENT.rgb * _MainColour.rgb;

				float3 diffuseReflection =
					attenuation * _LightColor0.rgb * _MainColour.rgb
				...
				else
				{
					specularReflection = attenuation * _LightColor0.rgb
						* _SpecularColour.rgb * pow(max(0.0, dot(
							reflect(-lightDirection, normalDirection),
							viewDirection)), _Lustre);
				}
